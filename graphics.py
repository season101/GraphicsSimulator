...
########## LIST AND DICTIONARY WRITTEN BY TYLER GREER ###########

commands = ["Shift_R","BackSpace","Shift_L",
            "Control_L","Control_R","Caps_Lock",
            "Left","Down","Up","Right","End",
            "Next","Prior","Home","Delete","Escape",
            "Num_Lock","App","Insert","Win_L","F1",
...
####### END OF LIST AND DICTIONARY WRITTEN BY TYLER GREER #######


############## GRAPHICS FUNCTIONS BY TYLER GREER ##############

#### CREATE WINDOW ####
def createWindow(title, height, width, color):
    
    # create window
    window = GraphWin(title, height, width)

    # adjust coordinates
    window.setCoords(0,0,height,width)
...
## Create text
def createText(window, middleX, middleY, textContent, fontSize, style = 'normal'):
   
   # create header text
   text = Text(Point(middleX, middleY), "")
...
## Create rectangle
def createRectangle(window, lowerX, lowerY, upperX, upperY, fillColor = 'None', outline = 1, outlineColor = 'black'):
   
   
   # create rectangle
   rectangle = Rectangle(Point(lowerX, lowerY), Point(upperX, upperY))

   # format rectangle
   rectangle.setFill(fillColor)
   rectangle.setWidth(outline)
   rectangle.setOutline(outlineColor)

   # put rectangle on window
...
## Creates an input box with a prompt that allows the user to enter data.
def createInputBox(window, prompt, promptLocationY, boxColor, fontSize, style = 'normal'):

    boxWidth = 8  # set width of input box

    # determine prompt posistion
    promptLocationX = 5 + ((10 * (fontSize / 12)) * (len(prompt) / 2))
    
    
    # CREATE TEXT PROMPT
    text = Text(Point(promptLocationX , promptLocationY), "")
    ...
    # CREATE INPUT BOX; placed to the right of the prompt
    entry = Entry(Point(((promptLocationX + (5 + (promptLocationX - (5 * (fontSize / 12)) / 2)) +
                         (10 * (fontSize / 12)) * (boxWidth / 2)) - (10 * (fontSize / 12))),
                          promptLocationY), boxWidth)
    
  
    # set entry box color, style, and size
    entry.setFill(boxColor)
    entry.setStyle(style)
    entry.setSize(fontSize)
    
    
    # draw input box to window
    text.draw(window)
    entry.draw(window)
    ...
    while stop != 'Return':
        ...
        if stop not in commands:    # if stop is not one of the commands

            if stop in symbols:     # if stop is in the dictionary it's a symbol
               
                userInput += symbols[stop]   # add symbol to user input

            else:                   # if stop is a letter or number
                userInput += stop   # add letter/number to user input

        else:

            # remove characters if Backspace is pressed
            if stop == 'BackSpace' and len(userInput) > 0:
               
                userInput = userInput[ :-1]

        # get next character from entry box
        stop = window.getKey()

    return userInput   # return user input as a string

   
   

## Create a table that stores data
def createTable(*args):
   
   fontSize = args[4]
   style = args[5]

   columns = args[2]  # number of columns in table
   ...
   # determine column with the most data
   while columnData < len(args):

     if longest < len(args[columnData]):
         longest = len(args[columnData])
   ...
   x1 = 10  # right x coordinate
   x2 = 0   # left x coordinate

   currColumn = 6  # first column is 5th in the argument list
   ...
   while columns > 0:
     ...
     y2 = args[3]  # top y coordinate
     y1 = y2 - 20 - (10 * (fontSize / 12)) # bottom y coordinate
     
     # set correct column width
     x2 = (x1 + determineColumnWidth(args[currColumn]) * 10 ) + 20 + 5 * ((fontSize - 12) // 2) + 20

     print(x1, x2)

     # create table column header
     header = Rectangle(Point(x1, y1), Point(x2, y2))        

     # create table column of correct length (20 per row)
     table = Rectangle(Point(x1, y1 - (((longest - 1) * (20 + 5 * ((fontSize - 12) // 2))))), Point(x2, y1))

     # set color of columns
     header.setFill(args[1])
     table.setFill(args[1])
     
     # draw column to window
     header.draw(args[0])
     table.draw(args[0])

     # put data in column
     while rows < len(args[currColumn]):

         # create text
         if rows == 0:
            data = Text(Point((x1 + x2)/2, (y1 + y2)/2), "")      # header title
         else:
            data = Text(Point((x1 + x2)/2, (y1 + y2)/2 - 5), "")  # column data

         # set font size and style
         data.setSize(fontSize)
         data.setStyle(style)

         # write text and put on window
         data.setText(args[currColumn][rows])
         data.draw(args[0])

         rows += 1

         # move down to next row
         y1 -= 20 + 5 * ((fontSize - 12) // 2)
         y2 -= 20 + 5 * ((fontSize - 12) // 2)

     columns -= 1
     currColumn += 1

     # update column start location for next column
     x1 = x2
...
def determineColumnWidth(data):
    ...
    longest = len(str(data[0]))

    count = 1

    # determine which data entry is the longest
    while count < len(data):

        if longest < len(str(data[count])):
            longest = len(str(data[count]))

        count += 1

    return longest   # return length of longest value

########## END OF GRAPHICS FUNCTIONS WRITTEN BY TYLER GREER ###########
